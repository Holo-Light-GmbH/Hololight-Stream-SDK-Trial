...
namespace HoloLight.Isar.Native.ImageTracking
{
	...
	public class ImageTracker : MonoBehaviour
	{
		...
		private void SendImage()
		{
			...
			var length = info.Length + 4 * 4;
			byte[] bytes = new byte[length];
			var offset = 0;

			Array.Copy(BitConverter.GetBytes((int)MessageType.IMAGE_TO_TRACK), 0, bytes, offset, 4);
			offset += 4;

			Array.Copy(BitConverter.GetBytes(_image.serverId), 0, bytes, offset, 4);
			offset += 4;

			Array.Copy(BitConverter.GetBytes((int)_image.type), 0, bytes, offset, 4);
			offset += 4;

			Array.Copy(BitConverter.GetBytes(_image.width), 0, bytes, offset, 4);
			offset += 4;

			Array.Copy(info, 0, bytes, offset, info.Length);

			SendChunked(bytes);
		}

		private void SendChunked(byte[] bytes)
		{
			int counter = 0;

			const int chunkSize = 8 * (1 << 10); // 8 kb

			for (int sent = 0; sent < bytes.Length;)
			{
				counter += 1;
				byte[] headerBytes = new MessageHeader(messageId: _image.serverId, totalSize: bytes.Length, position: sent).getBytes();
				int thisChunkSize = Math.Min(chunkSize, bytes.Length - sent);

				IntPtr unmanagedPtr = Marshal.AllocHGlobal(thisChunkSize + headerBytes.Length);
				Marshal.Copy(headerBytes, 0, unmanagedPtr, headerBytes.Length);
				Marshal.Copy(bytes, sent, IntPtr.Add(unmanagedPtr, headerBytes.Length), thisChunkSize);

				HlrCustomMessage message = new HlrCustomMessage();
				message.Length = (uint)(thisChunkSize + headerBytes.Length);
		...
		private void OnCustomMessageReceived(in HlrCustomMessage message)
		{
			...
			if (messageTypeEnum != MessageType.IMAGE_DATA)
				return;
