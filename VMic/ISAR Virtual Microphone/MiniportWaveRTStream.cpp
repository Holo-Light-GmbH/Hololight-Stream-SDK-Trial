...
#define HNSTIME_PER_MILLISECOND 200
...
--*/
{
	...
	if (m_PairedStream)
	{
		m_PairedStream->m_PairedStream = NULL;
	}
	...
	case KSSTATE_RUN:
		...
		if (m_ulNotificationIntervalMs > 0)
		{
			...
			(
				...
				HNSTIME_PER_MILLISECOND, // 0.02 ms 
				NULL
			);
...
)
{
	...
	if (m_bCapture)
	{
		...
		// Microphone end places displaced bytes to DMA buffer, speaker end will write to ring buffer of microphone end in its 
		// timer tick and once microphone end reaches here by its own timer tick it will transfer written bytes to DMA buffer 
	...
	else
	{
		...
		// This part gets called for speaker end
		// Read from DMA buffer and write to other stream-end's ring buffer (Microphone end's buffer)
...
This function writes the audio buffer, it takes displaced bytes since last tick from m_RingBuffer
and places them into m_pDmaBuffer which is direct memory access buffer created for kernel access.
Arguments:
...
This function reads the audio buffer and writes the audio data to other end (microphone).
